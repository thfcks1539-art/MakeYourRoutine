...
async function updateStreak(studentId, routineId, date, completedNow) {
  ...
  const row = await db.prepare(`SELECT * FROM streaks WHERE student_id = ? AND routine_id = ?`).get(studentId, routineId);
  const yesterday = addDays(date, -1);
  if (!row) {
    await db.prepare(`INSERT INTO streaks (student_id, routine_id, current_streak, best_streak, last_completed_date) VALUES (?, ?, 1, 1, ?)`)
  ...
  await db.prepare(`UPDATE streaks SET current_streak = ?, best_streak = ?, last_completed_date = ? WHERE student_id = ? AND routine_id = ?`)
...
router.post('/toggle', async (req, res) => {
  ...
  await db.prepare(`UPDATE routine_checks SET count = ?, completed = ?, completed_at = ? WHERE id = ?`)
    .run(count, completed, completedAt, row.id);

  if (completed === 1 && !row.completed) {
    await db.prepare(`UPDATE students SET points = points + 1 WHERE id = ?`).run(student_id);
  } else if (completed === 0 && row.completed) {
    await db.prepare(`UPDATE students SET points = MAX(points - 1, 0) WHERE id = ?`).run(student_id);
  }

  await updateStreak(student_id, routine_id, date, completed === 1);

  const student = await db.prepare(`SELECT points FROM students WHERE id = ?`).get(student_id);
